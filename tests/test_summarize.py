...
def test_clean_sentence() -> None:
    ...
    assert clean_sentence(
        [word.casefold() for word in nltk.word_tokenize('They may be replaced by new “federated” platforms.')]) == [
               'they', 'may', 'be', 'replaced', 'by', 'new', 'federated', 'platforms']
    assert clean_sentence([word.casefold() for word in nltk.word_tokenize(
        'Jürgen Habermas, the German philosopher who coined the term _legitimization crisis_,')]) == ['jürgen',
                                                                                                      'habermas', 'the',
                                                                                                      'german',
                                                                                                      'philosopher',
                                                                                                      'who', 'coined',
                                                                                                      ...
                                                                                                      'legitimization',
                                                                                                      'crisis']
    assert clean_sentence([word.casefold() for word in nltk.word_tokenize('etc. and/the mr.')]) == ['etc', 'and', 'the',
                                                                                                    'mr']
...
def test_clean_text() -> None:
    ...
    assert clean_text(['They may be replaced by new “federated” platforms.']) == [
        ['they', 'may', 'be', 'replaced', 'by', 'new', 'federated', 'platforms']]
    assert clean_text(['Illustration by Edmon de Haro',
                       'Jürgen Habermas, the German philosopher who coined the term _legitimization crisis_,']) == [
               ['illustration', 'by', 'edmon', 'de', 'haro'],
               ['jürgen', 'habermas', 'the', 'german', 'philosopher', 'who', 'coined', 'the', 'term', 'legitimization',
                'crisis']]
    assert clean_text(['etc. and/the mr.', 'To/from/cube [qr] i/o?', '(the#get&^where}']) == [
        ['etc', 'and', 'the', 'mr'], ['to', 'from', 'cube', 'qr', 'i', 'o'], ['the', 'get', 'where']]
...
def test_calculate_tf_idf() -> None:
    ...
    assert calculate_tf_idf({'back': 3.0, 'line': 2.0, 'test': 4.0}, {'test': 2.0, 'back': 1.0, 'line': 3.0}) == {
        'back': 3.0, 'line': 6.0, 'test': 8.0}
    assert calculate_tf_idf({'back': 3.0, 'line': 2.0, 'test': 4.0},
                            {'skd': 1.2, 'test': 2.0, 'sda': 5.4, 'back': 1.0, 'line': 3.0, 'lkj': 9.8}) == {
               'back': 3.0, 'line': 6.0, 'test': 8.0}
...
def test_score_sentences() -> None:
    ...
    assert isclose(test[7], 0.17909274372242093)
    assert isclose(test[14], 0.10086149660969546)
    assert len(test) == len(clean)
